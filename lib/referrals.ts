...
export const referralStore: ReferralStore = {
  async saveCode(code: ReferralCode) {
    const { error } = await getSupabaseAdmin().from("referral_codes").insert({
  ...
  async getCode(code: string): Promise<ReferralCode | null> {
    const { data } = await getSupabaseAdmin().from("referral_codes").select("*").eq("code", code).maybeSingle();
    if (!data) return null;
  ...
  async saveUsage(usage: ReferralUsage) {
    const { error } = await getSupabaseAdmin().from("referral_usages").insert({
  ...
  async getUsagesByAffiliate(affiliateId: string): Promise<ReferralUsage[]> {
    const { data } = await getSupabaseAdmin().from("referral_usages").select("*").eq("affiliate_id", affiliateId).order("applied_at", { ascending: false });
