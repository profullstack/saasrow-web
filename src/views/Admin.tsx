...
export default function AdminPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      tokens?.forEach((token: { email: string; tier: string; created_at?: string; expires_at?: string | null; last_used_at?: string | null }) => {
        const email = token.email.toLowerCase()
        const existing = emailMap.get(email)
        if (existing) {
          existing.tier = token.tier as 'free' | 'featured' | 'premium'
          existing.expires_at = token.expires_at ?? null
          existing.last_used_at = token.last_used_at ?? null
        } else {
          emailMap.set(email, {
            email: token.email,
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      setAlertMessage({ type: 'error', message: error instanceof Error ? error.message : 'Failed to load analytics data' })
    } finally {
