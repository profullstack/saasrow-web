...
export default function SoftwareDetailPage({ initialSubmission }: SoftwareDetailPageProps = {}) {
  const params = useParams<{ id: string }>()
  const id = params?.id ?? initialSubmission?.id
  const [submission, setSubmission] = useState<Submission | null>(initialSubmission ?? null)
  const [loading, setLoading] = useState(initialSubmission === undefined)
  const [notFound, setNotFound] = useState(initialSubmission === null)
  const [alertMessage, setAlertMessage] = useState<{ type: 'success' | 'error' | 'info' | 'warning'; message: string } | null>(null)
  const [userVote, setUserVote] = useState<'upvote' | 'downvote' | null>(null)
  const [upvotes, setUpvotes] = useState(initialSubmission?.upvotes ?? 0)
  const [downvotes, setDownvotes] = useState(initialSubmission?.downvotes ?? 0)
  const [isVoting, setIsVoting] = useState(false)
  const [shareCount, setShareCount] = useState(initialSubmission?.share_count ?? 0)
  ...
  useEffect(() => {
    if (initialSubmission === undefined) {
      ...
      checkTrendingStatus(initialSubmission)
    }
    if (id) {
        ...
        <main className="w-full max-w-[900px] mx-auto px-4 py-12">
          {loading ? (
            ...
            <>
              <div className="mb-8">
                <Link
                  href={`/category/${submission.category.toLowerCase()}`}
