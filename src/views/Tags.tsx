...
interface TagsPageProps {
  ...
  initialSoftware?: Submission[]
}

export default function TagsPage({ initialTags, initialSoftware }: TagsPageProps = {}) {
  const params = useParams<{ tag?: string }>()
  const tag = typeof params?.tag === 'string' ? params.tag : undefined
  const [tags, setTags] = useState<TagCount[]>(initialTags ?? [])
  const [software, setSoftware] = useState<Submission[]>(initialSoftware ?? [])
  const [loading, setLoading] = useState(
    tag ? initialSoftware === undefined : initialTags === undefined,
  )
  const [sortBy, setSortBy] = useState('popularity')

  useEffect(() => {
    if (tag) {
      if (initialSoftware !== undefined) return
      fetchSoftwareByTag(tag)
    } else {
      if (initialTags !== undefined) return
      fetchTags()
    }
  }, [tag, initialSoftware, initialTags])
        ...
        <div className="relative z-10">
          ...
          <main className="w-full max-w-[1200px] mx-auto px-4 py-12">
            <div className="text-center mb-12">
              <Link href="/tags" className="inline-block mb-4 text-[#4FFFE3] hover:underline font-ubuntu">
                ← Back to all tags
              </Link>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tags.map((tagItem, index) => (
                <Link
                  key={tagItem.name}
                  href={`/tags/${tagItem.name.toLowerCase()}`}
